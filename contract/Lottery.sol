...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("2local Lottery Ticket", "LLT") {}
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function buy(uint256 _price, uint8[6] memory _numbers) external {
        ...
        lastTimestamp = block.timestamp;
        local.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[6][] memory _numbers) external {
        ...
        local.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward > 0) {
            local.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward > 0) {
            local.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        local.safeTransfer(address(msg.sender), _amount);
