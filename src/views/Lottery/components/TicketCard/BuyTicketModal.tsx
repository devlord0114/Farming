...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  }, [max, tokenCountPerTicket])

  const handleChange = (e: React.FormEvent<HTMLInputElement>) => setVal(e.currentTarget.value)

  const { onMultiBuy } = useMultiBuyLottery()
  const maxNumber = useMaxNumber() - 1
  const handleBuy = useCallback(async () => {
    try {
      ...
      const txHash = await onMultiBuy(tokenCountPerTicket, numbers)
  ...
  }, [onMultiBuy, setRequestedBuy, maxNumber, val, tokenCountPerTicket])
  ...
  const cakeCosts = (amount: string): number => {
    return (+amount * tokenCountPerTicket.toNumber())
  }

  const cakeAmount = () => {
    return tokenCountPerTicket.toNumber().toFixed(2)
  }
